<?php
    $tBranchAddrRefCode   = $tCstBchCode;
    if(isset($nStaCallView) && $nStaCallView == 1){
        $tBranchAddrRoute ="customerBchAddrEventAdd";
    }else{
        $tBranchAddrRoute ="customerBchAddrEventEdit";
    }
    ...
    if(isset($aCstBchDataVersion) && !empty($aCstBchDataVersion)){
        if(isset($aCstBchDataVersion['FTSysStaUsrValue']) && !empty($aCstBchDataVersion['FTSysStaUsrValue'])){
            $nStaVersionAddress = $aCstBchDataVersion['FTSysStaUsrValue']; // ใช้ที่อยู่แบบที่ User กำหนด
        }else{
            $nStaVersionAddress = $aCstBchDataVersion['FTSysStaDefValue']; // ใช้ที่อยู่แบบที่ Systems กำหนด
        }
    }else{
...
<form id="ofmBranchAddressForm" class="validate-form" action="javascript:void(0)" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" id="ohdBranchAddressCbrRefBch"  name="ohdBranchAddressCbrRefBch"     value="<?php echo @$tCstBchCode;?>">   
