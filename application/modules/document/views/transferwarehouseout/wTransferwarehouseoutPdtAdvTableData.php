        ...
        <table id="otbTWODocPdtAdvTableList" class="table xWPdtTableFont">
            <thead>
                <tr class="xCNCenter">
                    ...
                    <?php endforeach;?>
                    <th nowrap class="xCNTextWOld xPIImportDT" style="display:none"><?php echo language('document/purchaseorder/purchaseorder','tPOLabelFrmInfoOthRemark')?></th>
            ...
            <tbody id="odvtbodyPIPdtAdvTableList">
                ...
                ?>
                        ...
                        >
                            <td nowrap align="center">
                                <label class="fancy-checkbox">
                                    <input id="ocbListItem<?=$nKey?>" type="checkbox" class="ocbListItem" name="ocbListItem[]" onclick="FSxTWOSelectMulDel(this)">
                            ...
                            <td nowrap="" class="text-center xCNPIBeHideMQSS">
                                <label class="xCNTextLink">
                                    <img class="xCNIconTable xCNIconDel" src="application/modules/common/assets/images/icons/delete.png" title="Remove" onclick="JSnTWODelPdtInDTTempSingle(this)" >
                                </label>
                            </td>
                        </tr>
                    <?php
                    ...
                    ?>
                    <tr><td class="text-center xCNTextDetail2 xWTwoTextNotfoundDataPdtTable" colspan="100%"><?php echo language('common/main/main','tCMNNotFoundData')?></td></tr>
                <?php endif;?>
            </tbody>
        </table>
    </div>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="table-responsive" style="min-height: 300px; max-height: 300px; overflow-y: scroll;">
                            <table id="otbDisChgDataDocHDList" class="table">
                                <thead>
                                    <tr class="xCNCenter">
                                        <th class="xCNTextWOld"><?=language('document/purchaseinvoice/purchaseinvoice','tPIsequence')?></th>
                                        <th class="xCNTextWOld"><?=language('document/purchaseinvoice/purchaseinvoice','tPIBeforereducing')?></th>
                                        <th class="xCNTextWOld"><?=language('document/purchaseinvoice/purchaseinvoice','tPIValuereducingcharging')?></th>
                                        <th class="xCNTextWOld"><?=language('document/purchaseinvoice/purchaseinvoice','tPIAfterReducing')?></th>
                                        <th class="xCNTextWOld"><?=language('document/purchaseinvoice/purchaseinvoice','tPIType')?></th>
                                        <th class="xCNTextWOld"><?=language('document/purchaseinvoice/purchaseinvoice','tPIDiscountcharge')?></th>
                                        <th class="xCNTextWOld"><?=language('document/purchaseinvoice/purchaseinvoice','tPITBDelete')?></th>
                                    </tr>
                                </thead>
                                <tbody>
...
<script>
    ...
    function  FSvTWONextFuncB4SelPDT(ptPdtData){
        ...
        for(var i=0;i<aPackData.length;i++){
            ...
            FSvTWOAddPdtIntoDocDTTempScan(aNewPackData);   // Append HMTL
            JSvTWOInsertPdtToTemp(aNewPackData);   // Insert Database
...
function JSvTWOInsertPdtToTemp_PDTSerialorFashion(ptPdtDataFhn){
//   console.log(ptPdtDataFhn);
 ...
 var tOptionAddPdt = $('#ocmTWOOptionAddPdt').val();
  $.ajax({
      type: "POST",
      ...
      success: function(oResult) {
          if( aData['nStaLastSeq'] == '1' ){ //ถ้าเป็นสินค้าแฟชั่นตัวสุดท้ายให้โหลด Table 
            //     JSvTRNLoadPdtDataTableHtml(1, true);;
            JSvTRNLoadPdtDataTableHtml(1, true);
          }
      },  
...
function FSvTWOPDTEditPdtIntoTableDT_PDTSerialorFashion(ptPdtDataFhn){
  ...
  $.ajax({
    ...
    // Append PDT
    function FSvTWOAddPdtIntoDocDTTempScan(ptPdtData){
        JCNxCloseLoading();
        var aPackData = JSON.parse(ptPdtData);
        var tCheckIteminTableClass = $('#otbTWODocPdtAdvTableList tbody tr td').hasClass('xWTwoTextNotfoundDataPdtTable');

        if(tCheckIteminTableClass == true){
            $('#otbTWODocPdtAdvTableList tbody').html('');
            var nKey    = 1;
        }else{
            var nKey    = parseInt($('#otbTWODocPdtAdvTableList tr:last').attr('data-seqno')) + parseInt(1);
        }
        ...
        for(var i=0; i<nLen; i++){
            ...
            var tDuplicate = $('#otbTWODocPdtAdvTableList tbody tr').hasClass('otr'+tUnixPdtCodeRow);
            var InfoOthReAddPdt = $('#ocmTWOOptionAddPdt').val();
            ...
            if(tDuplicate == true && InfoOthReAddPdt==1){
                ...
                if(tPdtforSys!='FH'){
                    ...
                    var oQty = '<div class="xWEditInLine'+nKey+'">';
                        ...
                        var  aOptionFashion   = {
                            'tDocumentBranch' : $('#oetSOFrmBchCode').val(),
                            'tDocumentNumber' : $('#oetTWODocNo').val(),
            ...
            //    var aNewPackData = 'JSxUpdateProductSerialandFashion('+aNewPackData+')';
               var  oQty = '<div class="xWEditInLine'+nKey+'">';       
                ...
                tHTML += '      <input id="ocbListItem'+nKey+'" type="checkbox" class="ocbListItem" name="ocbListItem[]" onclick="FSxTWOSelectMulDel(this)">';
                ...
                tHTML += '      <img class="xCNIconTable" src="application/modules/common/assets/images/icons/delete.png" title="Remove" onclick="JSxTWODataTableDeleteBySeq(this)">';
                tHTML += '  </label>';
                tHTML += '</td>';
                tHTML += '</tr>';
                nKey++;
        ...
        //สร้างตาราง
        $('#otbTWODocPdtAdvTableList tbody').append(tHTML);
    ...
    function FSxTWOSelectMulDel(ptElm){

        let tTwoDocNo    = $('#oetTWODocNo').val();
        let tTwoSeqNo    = $(ptElm).parents('.xWPdtItem').data('key');
        let tTwoPdtCode  = $(ptElm).parents('.xWPdtItem').data('pdtcode');
        let tTwoBarCode  = $(ptElm).parents('.xWPdtItem').data('barcode');
        let tTwoPunCode  = $(ptElm).parents('.xWPdtItem').data('puncode');
        // alert(1111);
    ...
    function JSxAddScollBarInTablePdt(){
        $('#otbTWODocPdtAdvTableList >tbody >tr').css('background-color','#ffffff');
        var rowCount = $('#otbTWODocPdtAdvTableList >tbody >tr').length;
        if(rowCount >= 2){
            $('#otbTWODocPdtAdvTableList >tbody >tr').last().css('background-color','rgb(226, 243, 255)');
        ...
        if(rowCount >= 7){
            ...
            // $('#otbTWODocPdtAdvTableList').height()
            $('.xCNTablescroll').scrollTop($('#otbTWODocPdtAdvTableList').height());
            $('html, body').animate({
             scrollTop: ($("#oetTwoInsertBarcode").offset().top)-80
         }, 0);
        }
