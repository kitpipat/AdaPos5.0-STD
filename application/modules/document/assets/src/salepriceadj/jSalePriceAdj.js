        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                if (jqXHR.status != 404){
                    ...
                    var tDisplayEvent = 'เรียกดูเอกสารใบปรับราคาขาย';
                    var tErrorStatus  = jqXHR.status;
                    var tLogDocNo   = ptXphDocNo;
...
function JSoAddEditSpa(ptRoute) {
    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        var aArrayPdt = [];
        var bCheckPDTDup = false;
        var tChkPdtName =''; 

        $("#otbSpaDataList tbody tr ").each(function( index ) {
            // alert($(this).eq(2).find('label').text());
        ...
        if(bCheckPDTDup == true){
            FSvCMNSetMsgWarningDialog('สินค้า ' + tChkPdtName+' พบรายการซ้ำ');
            return;
        }

        var nSUMPrice = 0;
        var bCheckPDT = false;
        $("#otbSpaDataList tbody td.text-right").each(function( index ) {
            nSUMPrice = parseInt($(this).find('.xStaDocEdit').val());
            if(nSUMPrice == 0){
                bCheckPDT = true;
                return;
            }
        ...
        var nSUMRmk = '';
        var bCheckRmk = false;
        $("#otbSpaDataList tbody td.xCNAdjPriceStaRmk").each(function( index ) {
            nSUMRmk = $(this).text();
            if(nSUMRmk != ''){
                bCheckRmk = true;
                return;
        ...
        if(bCheckRmk == true){
            FSvCMNSetMsgWarningDialog('มีสินค้าบางรายการไม่ถูกต้อง');
            return;
        }
        ...
        if (StopDate < StartDate) {
            ...
            $('#ofmAddSpa').validate({
                ...
                submitHandler: function(form) {
                    ...
                    $('.xWEditInlineElement').removeAttr('disabled', true);
                    $('#oetXphDocDate').removeAttr('disabled', true);
                    $('#oetXphDocTime').removeAttr('disabled', true);
                    $('#obtXphDocDate').removeAttr('disabled', true);
                    $('#obtXphDocTime').removeAttr('disabled', true);
                    $('#ocmXphDocType').removeAttr('disabled', true);
                    $('#ocmXphStaAdj').removeAttr('disabled', true);
                    $('#oetValue').removeAttr('disabled', true);
                    $('#ocmChangePrice').removeAttr('disabled', true);
                    $('#obtAdjAll').removeAttr('disabled', true);
                    $('#btnBrowseZone').removeAttr('disabled', true);
                    $('#btnBrowseBranch').removeAttr('disabled', true);
                    $('#btnBrowseMerChrant').removeAttr('disabled', true);
                    $('#btnBrowsePdtPriList').removeAttr('disabled', true);
                    $('#btnBrowseMerchant').removeAttr('disabled', true);
                    $('#oetXphDStart').removeAttr('disabled', true);
                    $('#obtXphDStart').removeAttr('disabled', true);
                    $('#oetXphDStop').removeAttr('disabled', true);
                    $('#obtXphDStop').removeAttr('disabled', true);
                    $('#oetXphName').removeAttr('disabled', true);
                    $('#oetXphRefInt').removeAttr('disabled', true);
                    $('#oetXphRefIntDate').removeAttr('disabled', true);
                    $('#obtXphRefIntDate').removeAttr('disabled', true);
                    $('#ocmXphPriType').removeAttr('disabled', true);
                    $('#ocbXphStaDocAct').removeAttr('disabled', true);
                    ...
                    $.ajax({
                        ...
                        success: function(oResult) {
                            ...
                            $('#oetXphTStart').removeAttr('disabled', true);
                            $('#obtXphTStart').removeAttr('disabled', true);
                        
                            $('#oetXphTStop').removeAttr('disabled', true);
                            $('#obtXphTStop').removeAttr('disabled', true);
                            ...
                            $('.xCNImportBtn').removeAttr('disabled', true);
                            $('.xCNBTNMngTable').removeAttr('disabled', true);
                            $('.xCNImportBtn').hide();
                            $('.xCNBTNMngTable').hide();
                            ...
                            if (nStaSpaBrowseType != 1) {
                                if (aReturn['nStaEvent'] == 1) {
                                    ...
                                    JCNxUPFInsertDataFile(oSPACallDataTableFile);
                        ...
                        error: function(jqXHR, textStatus, errorThrown) {
                            ...
                            if (jqXHR.status != 404){
                                var tLogFunction = 'ERROR';
                                var tDisplayEvent = 'บันทึก/แก้ไข ใบปรับราคาขาย ';
                                var tErrorStatus = jqXHR.status;
                                var tHtmlError = $(jqXHR.responseText);
                                var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
...
function JSxCheckDocType(pnSelected) {
    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        } else if (pnSelected == 2 || pnSelected == 3 || pnSelected == 4) {
            for (var i = 1; i <= 5; i++) {
...
//Function Chack Value Type
function JSxEditPun(pnValue, oElm) {
    var tSeq = $(oElm).attr('seq');
    var tValue = pnValue;
    var tDocNo = $('#otrSpaPdtPri' + tSeq).data('doc');

    $.ajax({
        type: "POST",
        url: "dcmSPAPdtPriEventUpdPriPunTmp",
        data: {
            'FTXthDocNo': tDocNo,
            'ptValue': tValue,
            'tSeq': tSeq
        ...
        success: function(pResutl) {
            ...
            var result = 0;
            $("#ohdFCXtdPriceRet"+tSeq).val(numberWithCommas(result.toFixed(nDecimalShow)));
            $("#ohdFCXtdPriceRet"+tSeq).attr('b4value', result);
        },
        error: function(jqXHR, textStatus, errorThrown) {
            JCNxResponseError(jqXHR, textStatus, errorThrown);
        }
...
function JSxSPAShowOriginalPrice(pnSeq) {
    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        var tPplCode = $('#oetPplCode').val();
        var tPplName = $('#oetPplName').val();
        // if(tAgg!=""){
        ...
        $.ajax({
            ...
            data: {
                ptFTPdtCode: $('#olaFTPdtCode' + pnSeq).text(),
                ...
                tPplCode: tPplCode,
                tPplName: tPplName,
                ptFTPdtName: $('#olaFTPdtName' + pnSeq).text(),
            },
            cache: false,
            Timeout: 0,
            success: function(tResult) {
                ...
                $.ajax({
                    ...
                    success: function(oResult) {
                        ...
                        if (aResult['nStaEvent'] != "1") {
                            ...
                            if(aResult['tType'] == "1") {
                                FSvCMNSetMsgSucessDialog('อนุมัติเอกสารสำเร็จ');
                                JSvCallPageSpaEdit(aResult['tCodeReturn']);
                            }else{
                                JSoSPASubscribeMQ();
                            }
                        }
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        if (jqXHR.status != 404){
                            var tLogFunction = 'ERROR';
                            var tDisplayEvent = 'อนุมัติใบปรับราคาขาย ';
                            var tErrorStatus = jqXHR.status;
                            var tHtmlError = $(jqXHR.responseText);
                            var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                            var tLogDocNo = tDocNo;
    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                if (jqXHR.status != 404){
                    var tLogFunction = 'ERROR';
                    var tDisplayEvent = 'ยกเลิกใบปรับราคาขาย';
                    var tErrorStatus = jqXHR.status;
                    var tHtmlError = $(jqXHR.responseText);
                    var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                    var tLogDocNo = tDocNo;
