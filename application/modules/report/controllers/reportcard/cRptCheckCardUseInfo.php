    ...
    function FSvCCallRptRenderExcel(){
        ...
        if (isset($aDataReport['raItems']) && !empty($aDataReport['raItems'])) {
            foreach ($aDataReport['raItems'] as $nKey => $aValue) {
                ...
                $tTxnDocType    = $aValue['FTTxnDocType'];
                $tUsrName    = $aValue['FTUsrName'];
                $rtTxnDocDate    = $aValue['rtTxnDocDate'];
                ...
                switch($tExplodeStaAct[1]){
                    ...
                    switch($tTxnDocType){
                        case '1':
                            $tExplodeDocTypeName = language('report/report/report','tRPCCheckPrePaidDocType1');
                            break;
                        case '5':
                            $tExplodeDocTypeName = language('report/report/report','tRPCCheckPrePaidDocType5');
                            break;
                        default:
                            $tExplodeDocTypeName = "-";
                    }
                ...
                $values = [
                    ...
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell($tExplodeDocTypeName),
                    WriterEntityFactory::createCell(null),
                    WriterEntityFactory::createCell($tUsrName),
                    WriterEntityFactory::createCell(null),
