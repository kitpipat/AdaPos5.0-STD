...
class Rptdailysaleinfoanime_controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('report/reportdailysaleinfo/Rptdailysaleinfoanime_model');
    ...
    private function init()
    {
        $this->aText = [
            // Title
            'tTitleReport' => language('report/report/report', 'tRptDailySaleinfoTitle'),
            ...
            //รายงาน ข้อมูลการขาย
            'tRptGrpPdtType'        => language('report/report/report', 'tRptGrpPdtType'),//ประเภทสินค้า
            'tFTPbnName'        => language('report/report/report', 'tFTPbnName'),//ยี่ห้อ
            'tFTPdtName'        => language('report/report/report', 'tFTPdtName'),//ชื่อสินค้า
            'tFTPdtNameJP'        => language('report/report/report', 'tFTPdtNameJP'),//	ชื่อสินค้าภาษญี่ปุ่น
            'tRptBarCodeunit'        => language('report/report/report', 'tRptBarCodeunit'),//บาร์โค้ดสินค้า(ชิ้น)
            'tRptSbdReTailUnit'        => language('report/report/report', 'tRptSbdReTailUnit'),//ราคาขายต่อชิ้น
            'tRptQtyIncomeUnit'        => language('report/report/report', 'tRptQtyIncomeUnit'),//จำนวนที่รับเข้า(ชิ้น)
            'tRptQtySalUnit'           => language('report/report/report', 'tRptQtySalUnit'),//จำนวนที่ขาย(ชิ้น)	
            'tFCStfOnHandQty'           => language('report/report/report', 'tFCStfOnHandQty'),//ราคาต่อชิ้น
    ...
    public function index()
    {
        ...
        if (!empty($this->tRptExportType) && !empty($this->tRptCode)) {
            ...
            $this->Rptdailysaleinfoanime_model->FSnMExecStoreReport($this->aRptFilter);
    ...
    public function FSvCCallRptViewBeforePrint($paDataSwitchCase)
    {
        ...
        $aDataReport = $this->Rptdailysaleinfoanime_model->FSaMGetDataReport($aDataWhereRpt);
        ...
        $tRptView = JCNoHLoadViewAdvanceTable('report/datasources/reportdailysaleinfo', 'wReportdailysaleinfoHtml', $aDataViewPdt);
    ...
    public function FSoCChkDataReportInTableTemp($paDataSwitchCase)
    {
        try {
            ...
            $nDataCountPage = $this->Rptdailysaleinfoanime_model->FSnMCountDataReportAll($aDataCountData);
    ...
    public function FSvCCallRptRenderExcel()
    {
        ...
        $aCells = [
            WriterEntityFactory::createCell($this->aText['tRptGrpPdtType']),
            WriterEntityFactory::createCell($this->aText['tFTPbnName']),
            WriterEntityFactory::createCell($this->aText['tFTPdtName']),
            WriterEntityFactory::createCell($this->aText['tFTPdtNameJP']),
            WriterEntityFactory::createCell($this->aText['tRptBarCodeunit']),
            WriterEntityFactory::createCell($this->aText['tRptSbdReTailUnit']),
            WriterEntityFactory::createCell($this->aText['tRptQtyIncomeUnit']),
            WriterEntityFactory::createCell($this->aText['tRptQtySalUnit']),
            WriterEntityFactory::createCell($this->aText['tFCStfOnHandQty'])
        ];
        ...
        if (isset($aDataReport['aRptData']) && !empty($aDataReport['aRptData'])) {
            foreach ($aDataReport['aRptData'] as $nKey => $aValue) {
                ...
                $values = [
                    ...
                    WriterEntityFactory::createCell($aValue['FTPdtNameOth']),
                    WriterEntityFactory::createCell($aValue['FTBarCode']),
                    WriterEntityFactory::createCell(FCNnGetNumeric($aValue["FCPdtStkSetPrice"])),
                    WriterEntityFactory::createCell(FCNnGetNumeric($aValue["FCPdtStkQtyIn"])),
                    WriterEntityFactory::createCell(FCNnGetNumeric($aValue["FCPdtStkQtySale"])),
                ...
                if (($nKey + 1) == FCNnHSizeOf($aDataReport['aRptData'])) { //SumFooter
                    ...
                    $cPdtStkSetPrice_Footer    = $aValue["FCPdtStkSetPrice_Footer"];
                    $cPdtStkQtyIn_Footer       = $aValue["FCPdtStkQtyIn_Footer"];
                    $cPdtStkQtySale_Footer      = $aValue["FCPdtStkQtySale_Footer"];
                    $cStkQtyBal_Footer         = $aValue["FCStkQtyBal_Footer"];
                    $values = [
                        ...
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(null),
                        WriterEntityFactory::createCell(FCNnGetNumeric($cPdtStkSetPrice_Footer)),
                        WriterEntityFactory::createCell(FCNnGetNumeric($cPdtStkQtyIn_Footer)),
                        WriterEntityFactory::createCell(FCNnGetNumeric($cPdtStkQtySale_Footer)),
                        WriterEntityFactory::createCell(FCNnGetNumeric($cStkQtyBal_Footer))
                    ];
    ...
    public function FSoCCallRptRenderFooterExcel()
    {
        ...
        if ((isset($this->aRptFilter['tPdtTypeCodeFrom']) && !empty($this->aRptFilter['tPdtTypeCodeFrom'])) && (isset($this->aRptFilter['tPdtTypeCodeTo']) && !empty($this->aRptFilter['tPdtTypeCodeTo']))) {
            $aCells = [
                WriterEntityFactory::createCell($this->aText['tPdtTypeFrom'].' : '.$this->aRptFilter['tPdtTypeCodeFrom'].' '.$this->aText['tPdtTypeTo'].' : '.$this->aRptFilter['tPdtTypeCodeTo']),
                WriterEntityFactory::createCell(NULL),
                WriterEntityFactory::createCell(NULL),
                WriterEntityFactory::createCell(NULL),
                WriterEntityFactory::createCell(NULL),
