...
class Rptpursplbypdt_controller extends MX_Controller
{
    /**
     * ภาษา
     * @var array
    ...
    public function __construct()
    {
        ...
        $this->load->model('report/reportbuy/Rptpursplbypdt_model');
        // Init Report
        $this->init();
    ...
    private function init()
    {
        $this->aText = [
            // Title
              ...
              'tTitleReport' => language('report/report/report', 'tRptpursplbypdtTitle'),
    ...
    public function index()
    {
        ...
        if (!empty($this->tRptCode) && !empty($this->tRptExportType)) {
            ...
            $this->Rptpursplbypdt_model->FSnMExecStoreReport($this->aRptFilter);
            // Count Rows
            $aCountRowParams = [
            ...
            $this->nRows = $this->Rptpursplbypdt_model->FSnMCountRowInTemp($aCountRowParams);
    ...
    public function FSvCCallRptViewBeforePrint()
    {
        try {
            ...
            $aDataReport = $this->Rptpursplbypdt_model->FSaMGetDataReport($aDataReportParams);
            ...
            $tRptView = JCNoHLoadViewAdvanceTable('report/datasources/reportbuy/', 'wRptpursplbypdt', $aDataViewRptParams);
    ...
    public function FSvCCallRptViewBeforePrintClickPage()
    {
        ...
        $aDataReport = $this->Rptpursplbypdt_model->FSaMGetDataReport($aDataWhere, $aDataFilter);
        // print_r($aDataReport);
        // exit;

        // GetDataSumFootReport
        // $aDataSumFoot = $this->Rptpursplbypdt_model->FSaMGetDataSumFootReport($aDataWhere, $aDataFilter);
        ...
        $tViewRenderKool = JCNoHLoadViewAdvanceTable('report/datasources/reportbuy/', 'wRptpursplbypdt', $aDataViewRptParams);
    ...
    public function FSoCChkDataReportInTableTemp()
    {
        try {
            ...
            $nDataCountPage = $this->Rptpursplbypdt_model->FSnMCountRowInTemp($aDataCountData);
    ...
    public function  FSvCCallRptRenderExcel()
    {
        ...
        if (isset($aDataReport['aRptData']) && !empty($aDataReport['aRptData'])) {
            ...
            foreach ($aSplName as $nKey => $aValue) {
              ...
              $tHeadtr = "ผู้จำหน่าย ".$aValue[0];
              $values = [
                  WriterEntityFactory::createCell($tHeadtr),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(NULL),
              ...
              $nFCXpdNetAmt = 0;
              $nFCXpdQty = 0;
              foreach ($aData[$nKey] as $key => $aValues) {
                $npdValue = $aValues['FCXpdValue'];
                $npdDis =$aValues['FCXpdDis'];
                $npdVat =$aValues['FCXpdVat'];
                $npdNetAmt =$aValues['FCXpdNetAmt'];
                $values = [
                    WriterEntityFactory::createCell($aValues['FTPdtCode']),
                    WriterEntityFactory::createCell(NULL),
                    WriterEntityFactory::createCell($aValues['FTPdtName']),
                    WriterEntityFactory::createCell(NULL),
                    WriterEntityFactory::createCell(NULL),
                    WriterEntityFactory::createCell(FCNnGetNumeric($aValues['FCXpdQty'])),
                    WriterEntityFactory::createCell(NULL),
                    WriterEntityFactory::createCell($aValues['FTPunName']),
                    WriterEntityFactory::createCell(NULL),
                    WriterEntityFactory::createCell(FCNnGetNumeric($npdValue)),
                    WriterEntityFactory::createCell(NULL),
                    WriterEntityFactory::createCell(FCNnGetNumeric($npdDis)),
                    WriterEntityFactory::createCell(NULL),
                    WriterEntityFactory::createCell(FCNnGetNumeric($npdVat)),
                    WriterEntityFactory::createCell(NULL),
                    WriterEntityFactory::createCell(FCNnGetNumeric($npdNetAmt))
                ];
                $aRow = WriterEntityFactory::createRow($values);
                $oWriter->addRow($aRow);

                $nFCXpdQty +=$aValues['FCXpdQty'];
                $nFCXpdValue += $aValues["FCXpdValue"];
                $nFCXpdDis += $aValues["FCXpdDis"];
                $nFCXpdVat += $aValues["FCXpdVat"];
                $nFCXpdNetAmt += $aValues["FCXpdNetAmt"];

                $nFCXpdValueSum +=$aValues['FCXpdValue'];
                $nFCXpdDisSum +=$aValues["FCXpdDis"];
                $nFCXpdVatSum +=$aValues["FCXpdVat"];
                $nFCXpdNetAmtSum +=$aValues["FCXpdNetAmt"];
                $nFCXpdQtySum +=$aValues["FCXpdQty"];
              }

              $values = [
                  WriterEntityFactory::createCell("รวม : ".$aValue[0]),
                  WriterEntityFactory::createCell(count($aData[$nKey])." รายการ"),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(NULL),
                  ...
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(FCNnGetNumeric($nFCXpdValue)),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(FCNnGetNumeric($nFCXpdDis)),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(FCNnGetNumeric($nFCXpdVat)),
                  WriterEntityFactory::createCell(NULL),
                  WriterEntityFactory::createCell(FCNnGetNumeric($nFCXpdNetAmt))
              ];
