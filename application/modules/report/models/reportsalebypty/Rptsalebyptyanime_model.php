...
class Rptsalebyptyanime_model extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{ CALL SP_RPTxSaleByPty_Animate(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tJoinFoooter   =   "   SELECT
                                        ...
                                        CONVERT(FLOAT,SUM(FCXsdCostAvg))              AS FCXsdCostAvg_Footer,
                                        CONVERT(FLOAT,SUM(FCXsdSaleTotal))                  AS FCXsdSaleTotal_Footer,
                                        CONVERT(FLOAT,SUM(FCStkCostBal))                 AS FCStkCostBal_Footer,
                                        CONVERT(FLOAT,SUM(FCSalAmtBal))                 AS FCSalAmtBal_Footer
                                    FROM TRPTSaleByPtyTmp_Animate WITH(NOLOCK)
                                    WHERE 1=1
            ...
            $tJoinFoooter  =   "   SELECT
                                        ...
                                        '0'           AS FCXsdCostAvg_Footer,
                                        '0'           AS FCXsdSaleTotal_Footer,
                                        '0'           AS FCStkCostBal_Footer,
                                        '0'           AS FCSalAmtBal_Footer
                                    ) T ON  L.FTUsrSession = T.FTUsrSession_Footer
            ";
        }
        ...
        $tSQL   = " SELECT
                    ...
                    FROM (
                        SELECT DISTINCT
                        ROW_NUMBER() OVER(ORDER BY DATA.FTPtyName ASC, DATA.FTRptRowSeq ASC) AS RowID,
                            DATA.*
                        FROM TRPTSaleByPtyTmp_Animate DATA WITH(NOLOCK)
                        WHERE 1=1
        ...
        $tSQL   .=  "   ORDER BY L.FTPtyName,L.FTRptRowSeq,L.FNRowPartID";
    ...
    public function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL           =   "   SELECT
                                ...
                                FROM TRPTSaleByPtyTmp_Animate STK WITH(NOLOCK)
                                WHERE 1=1
    ...
    public function FMxMRPTSetPriorityGroup($ptComName, $ptRptCode, $ptUsrSession)
    {
        $tSQLUPD   = "  UPDATE DATAUPD
                        ...
                        FROM TRPTSaleByPtyTmp_Animate DATAUPD
                        RIGHT JOIN (
                            SELECT 
                                ROW_NUMBER() OVER(PARTITION BY FTPtyName ORDER BY FTPtyName ASC,FTRptRowSeq ASC) AS PartIDPdt,
                                FTRptRowSeq,
                                FTPtyName,
                                FTComName,
                                FTRptCode,
    ...
    public function FSnMCountDataReportAll($paDataWhere)
    {
        ...
        $tSQL = "   
            SELECT 
                DTTMP.FTRptCode
            FROM TRPTSaleByPtyTmp_Animate AS DTTMP WITH(NOLOCK)
